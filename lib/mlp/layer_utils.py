...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        # Compute the gradient for the input features
...
class leaky_relu(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        # Compute the gradient for the Leaky ReLU activation
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        N = feat.shape[0]
        # Convert label to a one-hot encoded matrix
        label_one_hot = np.eye(logit.shape[1])[label]
        loss = -np.sum(label_one_hot * np.log(logit)) / N if self.size_average else -np.sum(label_one_hot * np.log(logit))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = logit
    ...
    def backward(self):
        ...
        # Convert label to a one-hot encoded matrix
        label_one_hot = np.eye(logit.shape[1])[label]
        dlogit = (logit - label_one_hot) / N if self.size_average else (logit - label_one_hot)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = None
...
def softmax(feat):
    ...
    #############################################################################
    exp_scores = np.exp(feat - np.max(feat, axis=-1, keepdims=True))
